import { getMyPastEvents } from "@/services/user/getMyPastEvents";
...
async function PastEventsContent({ searchParams }: PastEventsPageProps) {
    const params = await searchParams;
    const page = parseInt(params.page || "1", 10);
    const limit = 12;

    const eventsData = await getMyPastEvents({
        page,
        limit,
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-muted-foreground">
                    Events you've attended that have ended ({eventsData.meta.total} total)
                </p>
            </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {eventsData.data.map((event) => (
                        <Link
                            ...
                            className="group"
                        >
                            <Card className="h-full transition-all hover:shadow-md opacity-90 hover:opacity-100">
                                {event.image && (
...
const PastEventsPage = async (props: PastEventsPageProps) => {
    return (
